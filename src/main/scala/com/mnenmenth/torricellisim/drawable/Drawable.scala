...
trait Line extends Drawable {
  ...
  def begin_=(p: Point[Double])={
    _begin = p
    line.setLine(CoordSys.c2p(begin).toAwtPoint, CoordSys.c2p(end).toAwtPoint)
  }
  ...
  val line = new Line2D.Double(CoordSys.c2p(begin).toAwtPoint, CoordSys.c2p(end).toAwtPoint)
...
/**
  * Dotted Line generic
  */
trait DottedLine extends Drawable {
  private var _begin: Point[Double] = Point(0.0, 0.0)
  def begin = _begin
  def begin_=(p: Point[Double])={
    _begin = p
